...
(async()=>{
 ...
 try{
  ...
  await page.addInitScript(store=>{if(!localStorage.getItem('qualiopi-demo-v1'))localStorage.setItem('qualiopi-demo-v1',JSON.stringify(store));},store);
  ...
  await other.addInitScript(data=>localStorage.setItem('qualiopi-demo-v1',JSON.stringify(data)),persisted);
